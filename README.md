...
出力画像にはランダムの色で取得した矩形領域の枠が色付けられ、同じ色で番号が振られる。この番号は、ターミナルに出力された番号と一致する。4 つの数字は、対象の矩形領域の左上のx座標、y座標、右下のx座標、y座標の順に左上と右下の角が何ピクセルにあたるかを示している。すなわち、((左上x, 左上y), (右下x, 右下y)) という形で表示されている。
