...
if lines is not None:
    ...
    for i in range(rect_sorted_memory.shape[0]):
        ...
        for j, line in enumerate(line_nparray):
            ...
            if not ( (rect_sorted_memory[i][0][0] - error <= line_mid_x <= rect_sorted_memory[i][3][0] + error)
                ...
                is_underline = True
                print(f'before: {line_list[i]}')

            # 水平線の左端のy座標が、矩形領域の各辺の許容誤差内にあるかどうかを確認する
            if not ( (rect_sorted_memory[i][1][0] - error <= line_mid_x <= rect_sorted_memory[i][2][0] + error)
                ...
                is_underline = True
                print(f'after: {line_list[i]}')
                                                    
            # 重複フラグがTrueであれば、水平線は重複していないと判断し、リストに追加する
            if is_underline:
                underline_list.append(line_list[i])
                break
            
print(f'under: {underline_list}')
unique_horizontal_lines = underline_list
...
for i, line in enumerate(unique_horizontal_lines):
    ...
    # x1 = unique_horizontal_nparray[i][0]
    # y1 = unique_horizontal_nparray[i][1]
    # x2 = unique_horizontal_nparray[i][2]
    # y2 = unique_horizontal_nparray[i][3]
