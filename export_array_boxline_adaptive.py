...
cv2.imwrite(f'{results_path}/0_gray_blur.png', img_gray) # 確認用
...
img_bw = cv2.adaptiveThreshold(img_gray,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV,93,3)
...
# ハフ変換による直線検出
lines = []
lines = cv2.HoughLinesP(img_edges, rho=1, theta=np.pi/360, threshold=int(cv2.THRESH_OTSU), minLineLength=min_length, maxLineGap=1)
