...
def find_text_and_bounding_box(img_bw: np.ndarray, img_OCR: np.ndarray, file_name: str) -> Tuple[List[str], List[np.ndarray]]:
    ...
    text_deleted = [splitted_txt[i] for i in range(len(splitted_txt)) if i not in delete_index]
    bounding_box_deleted = [res[i].position for i in range(len(res)) if i not in delete_index] 
    ...
    return text_deleted, bounding_box_deleted
